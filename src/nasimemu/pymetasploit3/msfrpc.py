...
from nasimemu.pymetasploit3.utils import *
import requests
...
class MsfRpcClient(object):
    ...
    def post_request(self, url, payload):
        return self.session.post(url, data=payload, headers=self.headers, verify=False)
...
class MsfConsole(object):
    ...
    def run_module_with_output(self, mod, payload=None, run_as_job=False, timeout=301):
        ...
        while(self.is_busy()):
            print("Console busy. Waiting...")
            time.sleep(5) # wait for the console to clear

        self.write(options_str)
        old_data_len = 0
        data = ''
        timer = 5
        time.sleep(5)
        while data == '' or self.is_busy() or old_data_len != len(data):
            time.sleep(5)
